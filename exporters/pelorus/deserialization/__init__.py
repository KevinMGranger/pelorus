...
class _Deserializer:
    ...
    def _deserialize(self, src: Any, target_type) -> Optional[Any]:
        ...
        # do we treat `Optional` virally? each method has to handle it specially?
        # There must be a better way.
        # But if it can be arbitrarily nested, I guess not.
        # just like functionally, you'd have to handle it at each point.
        # right?

        if target_type is Any:
            return src

        # must test "non-classes" first.
        if (optional_alt := _is_optional(target_type)) is not None:
            return self._deserialize_optional(src, optional_alt)  # type: ignore
        ...
        if (dict_types := _is_dict_type(target_type)) is not None:
            if isinstance(src, Mapping):
                return self._deserialize_dict(src, dict_types[1])  # type: ignore
            else:
                raise TypeCheckError(Mapping, src)

        if (list_member_type := _is_list_type(target_type)) is not None:
            if isinstance(src, Iterable):
                return self._deserialize_list(src, list_member_type)  # type: ignore
            else:
    ...
    def _deserialize_dict(
        self, src: dict[str, Any], target_value_type: type[T]
    ) -> dict[str, T]:
        dict_ = {}
        errors = []
    ...
    def _deserialize_list(self, src: Any, target_value_type: type[T]) -> list[T]:
        list_ = []
        errors = []

        # TODO: how do we properly handle when it's not enumerable?
